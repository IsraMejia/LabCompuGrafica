...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
        ...
        // Recuperamos la posición exacta de la Articulación 3 (sin escala)
        model = modelaux; 

        // ---------------------------------------------------------
        //  TERCER BRAZO
        // ---------------------------------------------------------
        // Nos movemos hacia el centro del tercer brazo. Si va a medir 5 unidades, nos movemos 2.5
        model = glm::translate(model, glm::vec3(2.5f, 0.0f, 0.0f)); 
        modelaux = model; //  Guardamos el centro del Brazo 3 para usarlo como ancla de la siguiente articulación

        // Aplicamos la escala para darle forma alargada al brazo y lo dibujamos
        model = glm::scale(model, glm::vec3(5.0f, 1.0f, 1.0f));
        glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
        color = glm::vec3(1.0f, 1.0f, 0.0f); // Lo pintamos amarillo  
        ...
        // ARTICULACIÓN 4
        // ---------------------------------------------------------
		model = modelaux; // Regresamos al centro del Brazo 3  

		// Nos movemos a la punta derecha del Brazo 3  
		model = glm::translate(model, glm::vec3(2.5f, 0.0f, 0.0f));

		// Dejamos la articulación limpia, controlada solo por el teclado
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion4()), glm::vec3(0.0f, 0.0f, 1.0f));
		modelaux = model; // Guardamos el estado "puro" de la Articulación 4

		// Dibujar la esfera que representa la articulación
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		// ---------------------------------------------------------
		// 3. CANASTILLA (Cabina superior)
		// ---------------------------------------------------------
		model = modelaux; // Regresamos a la posición de la Articulación 4 (sin la escala de la esfera) 
		
		// Rotamos solo el sistema de coordenadas local de la canastilla
		model = glm::rotate(model, glm::radians(-40.0f), glm::vec3(0.0f, 0.0f, 1.0f));

		// Nos movemos ligeramente hacia arriba (sobre el nuevo eje Y rotado) para asentar la canastilla
		model = glm::translate(model, glm::vec3(0.0f, 1.0f, 0.0f));

		// Escala para darle forma de caja
		model = glm::scale(model, glm::vec3(1.5f, 1.5f, 1.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 1.0f, 1.0f); // Color Cyan
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[0]->RenderMesh(); // Dibuja el cubo (Canastilla)
